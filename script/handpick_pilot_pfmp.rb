...
pfmps.each do |pfmp|
  ...
  if payment.amount != expected_amount
    p "Update payment amount of pfmp ##{pfmp.id} from #{payment.amount} to #{expected_amount}"
  ...
  other_schoolings = pfmp.student.schoolings.where.not(id: pfmp.schooling_id).order(start_date: :asc)
