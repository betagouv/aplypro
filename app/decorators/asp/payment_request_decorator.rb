...
module ASP
  module PaymentRequestDecorator
    ...
    def status_badge
      dsfr_badge(status: BADGE_STATE_MAPPING[current_state.to_sym], html_attributes: { class: ["fr-badge--sm"] }) do
    ...
    def all_status_badges
      current_stages.map do |state|
        ...
        dsfr_badge(status: BADGE_STATE_MAPPING[state], html_attributes: { class: ["fr-badge--sm", disabled] }) do
          status(state)
        end
      end
    end
