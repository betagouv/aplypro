      ...
      class CorrectionFranceMapper < FranceMapper
        ...
        def cpltdistribution
          unsupported_voie_address ||
            [fallback_voie_bis, rnvp_data["ligne3"].presence].compact.join(" ").presence
        end

        def codepostalcedex
          rnvp_data["codePostal"]
        end

        def codecominsee
          insee_code = rnvp_data["codeInsee"].presence || student.address_city_insee_code
        ...
        def rnvp_data
          @rnvp_data ||= Rails.cache.read([ASP::RnvpEnricher::CACHE_KEY_PREFIX, student.id])
        end

        def voie_bis
          rnvp_data["voieBis"].presence&.upcase
        end

        def voie_type
          rnvp_data["voieType"].presence&.upcase
        end
        ...
        def unsupported_voie_address
          ...
          [rnvp_data["voieNum"], voie_type, rnvp_data["voieDen"]].compact.join(" ")
        end
      end
    end
  end
