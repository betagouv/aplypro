...
module ASP
  module Entities
    class AddressFixEnregistrement < Enregistrement
      ...
      def individu(xml)
        ...
        xml.listedossier { AddressFixDossier.from_payment_request(payment_request).to_xml(xml) }
      rescue ActiveModel::ValidationError => e
        Sentry.capture_exception(
          ASP::Errors::PaymentFileValidationError.new(
            "Address fix file validation failed for p_r: #{payment_request.id} with message #{e.message}"
          )
        )
        raise e
      end
